...
def update_boids(xs, ys, xvs, yvs,frame):
    ...
    collision = np.clip(VISIBLE_THRE - distance / COLLISION_THRE, 0,VISIBLE_THRE) * visible  # visible and in some distance 3*collision_thre
    c_tmp = np.diag(np.diag(collision))
    collision = collision - c_tmp

    if len(np.where(yvs[np.where(ys < VISIBLE_THRE*WALL_COLLISION_LIMIT)] == -1)[0])>0:
        wall_tmp=np.where(ys < VISIBLE_THRE*WALL_COLLISION_LIMIT)[0]
    ...
    if len(np.where(xvs[np.where(xs < VISIBLE_THRE*WALL_COLLISION_LIMIT)]==-1)[0])>0:
        wall_tmp = np.where(xs < VISIBLE_THRE*WALL_COLLISION_LIMIT)[0]
        for i_wall in range(len(wall_tmp)):
    ...
    if len(np.where(yvs[np.where((WORLD_WIDTH - ys) < VISIBLE_THRE * WALL_COLLISION_LIMIT)] == 1)[0]) > 0:
        wall_tmp = np.where((WORLD_WIDTH - ys) < VISIBLE_THRE * WALL_COLLISION_LIMIT)[0]
        for i_wall in range(len(wall_tmp)):
            if yvs[wall_tmp[i_wall]]==1:
    ...
    if len(np.where(xvs[np.where((WORLD_WIDTH - xs) < VISIBLE_THRE*WALL_COLLISION_LIMIT)] ==1)[0])>0:
        wall_tmp=np.where((WORLD_WIDTH - xs) < VISIBLE_THRE * WALL_COLLISION_LIMIT)[0]
        for i_wall in range(len(wall_tmp)):
            if xvs[wall_tmp[i_wall]]==1:
    ...
    print(col_c)
    col_c_tmp = np.clip(VISIBLE_THRE - col_c / WALL_COLLISION_LIMIT, 0, VISIBLE_THRE)
